...
class _ProfileScreenState extends State<OtherProfileScreen>
  ...
  void getProfile() async {
    ...
    final res = await UserApi()
        .getUserProfile(widget.userId, token, page, limit);
    if (res is String) {
      if (!mounted) return;
  ...
  void getPostProfile() async {
    ...
    final res = await UserApi()
        .getPostProfile(widget.userId, token, page, limit);
    if (res is String) {
      if (!mounted) return;
      showSnackBar(context, 'Error', res);
    } else {
                                  ...
                                  children: [
                                    ...[
                                      ...
                                      return StaggeredGridTile.count(
                                        ...
                                        mainAxisCellCount: 1,
                                        child: GestureDetector(
                                          onTap: () =>  Navigator.of(context).push(MaterialPageRoute(builder: (_) => ExploreListPostScreen(posts: _posts, title: 'Posts'))),
